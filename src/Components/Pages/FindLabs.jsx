...
function FindLabs() {
    ...
    async function fetchLabs() {
        const result = await getApiData(`lab?page=${currentPage}&limit=12&name=${name}&location=${location}`)
        if (result.success) {
            setLabs(result.data)
    ...
    async function fetchHospitals() {
        const result = await getApiData(`api/hospital?page=${currentPage}&limit=4&name=${name}&location=${location}`)
        if (result.success) {
            setHospitals(result.data)
            setHospitalPages(result.pagination.totalPages)
            setCurrentHospital(result.pagination.currentPage)
            setTotalHospital(result.pagination.total)
        }
    }
    useEffect(() => {
        fetchLabs()
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="text-center mb-3">
                                <h4 className="lg_title">Find Labs</h4>
                            </div>
                    ...
                    <div className="row">
                                            ...
                                            >
                                                <div className="accordion-body">
                                                    <ul className="permision-check-list">
                                                        <li>
                                                            <div className="accordion-body-concet">
                                                                <input className="form-check-input mt-0" type="radio"  onChange={()=>setType('hospital')} id="available" name="type" checked={type==='hospital'} value="hospital" aria-label="Checkbox for following text input" />
                                                                <label htmlFor="available">Hospital</label>
                                                            </div>
                                                        </li>

                                                        <li>
                                                            <div className="accordion-body-concet">
                                                                <input className="form-check-input mt-0" type="radio" id="not" name="type"
                                                                onChange={()=>setType('lab')} checked={type==='lab'} value="lab" aria-label="Checkbox for following text input" />
                                                                <label htmlFor="not">Individual</label>
                                                            </div>
                                                        </li>
                        ...
                        {type == 'lab' ?
                            ...
                            <div className="col-lg-9 col-md-12 col-sm-12">
                                ...
                                {hospitalPages > 1 && <div className="page-selector">
                                <div className="filters">
                                    <select className="form-select custom-page-dropdown nw-custom-page "
                                        value={currentHospital}
                                        onChange={(e) => setCurrentHospital(e.target.value)}>
                                        {Array.from({ length: hospitalPages }, (_, i) => (
                                            <option key={i + 1} value={i + 1}>{i + 1}</option>
                                        ))}
                                    </select>
                                </div>
                            ...
                            <div className="show-result-bx">
                                <p className="show-pra">Showing <span className="show-title">{totalHospital}</span> labs For You</p>
                            </div> */}
                                                    ...
                                                    <div className="doctor-mega-card">
                                                        <div className="doctor-pic-bx">
                                                            <img src={item?.logoFileId ?
                                                                `${base_url}/${item?.logoFileId}` : "/lab-pic.png"} alt="" />
                                                        </div>
                                                        <div className="doctor-details  flex-grow-1">
                                                            ...
                                                            <div className="  d-flex align-items-center justify-content-between">
                                                                <div>
                                                                    <Link to={`/lab-detail/${item?.hospitalName}/${item?._id}?type=hospital`} className="nw-thm-btn">View Details</Link>
                                                                </div>
                                                                <div>
                                                                    <button className="heart-btn" onClick={() => handleFavorite(item?._id)}>
                                                                        {favIds?.some(fav => fav?.labId === item?._id) ?
