...
export const fetchEmpDetail = createAsyncThunk(
    ...
    async (id, { rejectWithValue }) => {
        try {
            const response = await getSecureApiData(`doctor/staff-data/${id}`);
            if (response.success) {
                return response;
            }
        } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchEmpDetail.fulfilled, (state, action) => {
                state.loading = false;
                state.empData = action.payload.employee;
                state.isOwner = action.payload.empAccess.permissionId?.doctor ? false : true;
                state.permissions = action.payload.empAccess.permissionId?.doctor || null;
            });
    },
});
